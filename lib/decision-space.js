...
DecisionSpace.prototype = {
  ...
  build: function() {
    ...
    while (procQueue.length) {
      ...
      var [treeNode, constraintsWithValues] = procQueue.pop();

      // --- kDecisionCheckList/kDecisionResult case
      if (constraintsWithValues.length == 1) {
        var [transformedConstraints, thing] = constraintsWithValues[0];
      ...
      for (iCon = 0; iCon < constraintsWithValues.length; iCon++) {
        ...
        for (itCon = 0; itCon < constraintsWithValue.length; itCon++) {
          var tConTupe = constraintsWithValue[itCon];
    ...
    while (true) {
      ...
      if (aBranch[0] == this.kDecisionCheckList) {
        ...
        for (var i = 0; i < attrChecks.length; i++) {
          // attrChecks[1][0] is flatAttr
          var attrList = attrChecks[1][1];
          var value = attrChecks[1][2];
          if ((value !== this.WILD) &&
