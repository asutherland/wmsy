...
Partial.prototype = {
  evaluate: function(aObj) {
    let rval = this._space.evaluate(aObj, this._branch);
...
function DecisionSpace(aAttrPriorities) {
  ...
  if (aAttrPriorities) {
    for each (let [iAttr, attrName] in Iterator(aAttrPriorities)) {
...
DecisionSpace.prototype = {
  ...
  addPossibility: function(aConstraint, aThing) {
    this._possibilities.push([aConstraint, aThing]);
    this._dirty = true;
  },

  removePossibility: function(aThing) {
    for each (let [i, [constraint, conThing]] in Iterator(this._possibilities)){
      if (conThing == aThing) {
        this._possibilities.splice(i, 1);
        return;
      }
    }
  },
   ...
   *     traversal.
   */
  _flattenAndNormalize: function(aOutList, aObj, aWildAttrs, aParentPath) {
  ...
  build: function() {
    ...
    let allConstraintsWithValues = [];
    let attrsMarkedWild = {};
    for each (let [, [constraint, thing]] in Iterator(this._possibilities)) {
      let transformedConstraints = [];
    ...
    let procQueue = [[this._tree, allConstraintsWithValues]];
    ...
    while (procQueue.length) {
      ...
      let [treeNode, constraintsWithValues] = procQueue.pop();

      // --- kDecisionCheckList/kDecisionResult case
      if (constraintsWithValues.length == 1) {
        let [transformedConstraints, thing] = constraintsWithValues[0];
      ...
      let counts = {};
      let highCount = 0;
      let highPrio = this._lowest_priority;
      let highFlatAttr = null;
      let highAttrList = null;
      for each (let [, [tConstraints, thing]] in
                Iterator(constraintsWithValues)) {
        for each (let [, [flatAttr, attrList, value]] in
                  Iterator(tConstraints)) {
      ...
      //  the wild set.
      for each (let [, constraintsWithValue] in
                Iterator(constraintsWithValues)) {
        let containedAttr = false;
        for each (let [iConTupe, tConTupe] in
                  Iterator(constraintsWithValue[0])) {
          ...
          if (tConTupe[0] == highFlatAttr) {
            ...
            // need to create the entry in valueSet and note the new branch
              ...
              let branch = valueMap[value] = [];
              let cpairs = valueSet[value] = [constraintsWithValue];
...
//dump(JSON.stringify(this._tree, null, 2));
  ...
  _attrLookup: function(aObj, aAttrList) {
    if (aAttrList.length == 1)
      return (aAttrList[0] in aObj) ? aObj[aAttrList[0]] : undefined;

    for each (let [, attr] in Iterator(aAttrList)) {
   ...
   */
  evaluate: function(aObj, aBranch) {
    if (!aBranch)
      aBranch = this._tree;
    while (true) {
      ...
      if (aBranch[0] == this.kDecisionCheckList) {
        for each (let [, [flatAttr, attrList, value]] in Iterator(aBranch[1])) {
          // the value must match (or be WILD)
          if ((value !== this.WILD) &&
      ...
      // - kDecisionBranch case
      let val = this._attrLookup(aObj, aBranch[1]);
      // Missing branch?
      if (val === undefined) {
...
//dump("internal partial: " + JSON.stringify(aBranch, null, 2) + "\n");
      ...
      let attrPath = aBranch[1];
      let bail = false;
      for (let iPLen = 1; iPLen <= attrPath.length; iPLen++) {
      ...
      let val = this._attrLookup(aObj, attrPath);
   ...
   */
  partialEvaluate: function(aObjWillHave, aObjWillNeverHave) {
    let branch = this._tree ?
                   this._internalPartial(aObjWillHave, aObjWillNeverHave) :
                   null;
    let partial = new Partial(this, branch, aObjWillHave, aObjWillNeverHave);
    if (this._weakify)
   ...
   *  tree.
   */
  _updatePartials: function() {
    let weakify = this._weakify;
    let partials = [];
    for each (let [, weakPartial] in Iterator(this._partials)) {
      let partial = weakify ? weakPartial.get() : weakPartial;
      if (partial) {
