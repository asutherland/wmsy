 ...
 *       overlap.
 *     }
 *     @key["clickToFocus" #:optional Boolean]{
...
function WmsyExportDomain(aContext) {
  ...
  if ("clickToFocus" in aContext)
    this.clickToFocus = Boolean(aContext.clickToFocus);
  else
...
WmsyExportDomain.prototype = {
  ...
  /**
   * XXX text input box.  It's unclear if the binding being one way is
   *  acceptable.
   * XXX more focus unification required
   */
  text: function(aValueBinding, aDOMAttrBindings, aStaticBindings) {
    return new WmsyStructMarker("text", aDOMAttrBindings, aValueBinding,
                                aStaticBindings);
  },
  ...
  /**
   * Define an interposing view slice decorator.
   *
   * @args[
   ...
   *  This is currently being added for the jstut syntax highlighted display
   *  which creates styled nodes that are not wmsy widgets but needs styling.
   */
...
WmsyWrappedElement.prototype = {
  bind: function(aConstraintWithObj) {
