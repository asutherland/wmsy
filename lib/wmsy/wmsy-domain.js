...
var DecoratingInterposingViewSlice =
  require("wmsy/viewslice-interpose").DecoratingInterposingViewSlice;
...
WmsyDomain.prototype = {
   ...
   *
   * @consumes[WmsyObjDef.emit WmsyObjDef.receive WmsyObjDef.relay]
   */
   ...
   *   @param[aWidgetDef WidgetDef]{The widget definition}
   * ]
   */
...
console.log("WIDGET DEF: " + aWidgetDef.name);
    ...
    protoFab.proto.__flattenedCssString = cssBlocks.join("\n");
...
console.log("KEY EVENT!", aEvent.keyCode);
    ...
    switch (aEvent.keyCode) {
      // -- ENTER
      case 13:
        focusedBinding = focusManager.focusedBinding;
        if (focusedBinding)
  ...
  _handleEvent: function(aEvent, aTarget, aHandlerName, aSecondaryHandlerName) {
    ...
    for (var curNode = aTarget; curNode && (!("wmsyTop" in curNode));
      ...
      for (var iClass = 0; iClass < cssClassNames.length; iClass++) {
        ...
        if (aHandlerName in structInfo)
          structInfo[aHandlerName].call(bindingRootNode.binding, targetBinding,
                                        aEvent);
        else if (aSecondaryHandlerName && (aSecondaryHandlerName in structInfo))
          structInfo[aSecondaryHandlerName].call(
            bindingRootNode.binding, targetBinding, aEvent);
