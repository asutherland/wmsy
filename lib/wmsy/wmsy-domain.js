...
var DecoratingInterposingViewSlice =
      mod_vs_interpose.DecoratingInterposingViewSlice;

var FocusManager = wmsy_focus.FocusManager;

var WidgetProtoFab = wmsy_protofab.WidgetProtoFab;
...
WmsyDomain.prototype = {
    ...
    // helper tagged things
      ...
      //  perform fusion of nested objects as well.)
        ...
        if (aValue.args instanceof WmsyStructMarker) {
          ...
          if (decorMarker.kind === "interposing") {
            decorMarker.constructor = DecoratingInterposingViewSlice;
          }
          else {
            throw new Error(decorMarker.kind + " is not an acceptable view " +
        ...
        aProtoFab.addWidgetList(aKey, constraintBasis, separator,
                                kdata.subElementType, kdata.subElementCssClass,
                                kdata.listVertical, this, decorMarker,
                                aValue.kind == "stream");
      }
      ...
      else if (aValue.kind == "text" || aValue.kind == "password") {
        kdata.elementType = "input";
        kdata.children = null;
  ...
  _focusChew: function(aFocusDef, aStructMap, aProtoFab, aOrigin, aWidgetDef) {
    ...
    if (aFocusDef.kind == "item") {
      aProtoFab.makeFocusable(aOrigin.clickToFocus);
      return;
    }
    ...
    for (var i = 0; i < kidNames.length; i++) {
      if (!(kidNames[i] in aStructMap)) {
        throw new Error("you are claiming " + aWidgetDef.name +
                        " has a focusable child '" + kidNames[i] + "' but" +
                        " that is a lie.");
      }
...
//console.log("  REBUILT");
   ...
   */
  attachExportDomainToDocument: function(exportDomainQName, doc) {
   ...
   * }
   */
  bindObjectIntoNode: function(aConstraintWithObj, aDomElem) {
    ...
    if (!("wmsyInsertedExportDomains" in doc)) {
      ...
      // we expose the focus manager so widgets can find it.
  ...
  bindEventsOnRoot: function(aRootElem) {
    // simple re-binding suppression
    if ("_wmsyBound" in aRootElem)
      return;
      ...
      for (var iClass = 0; iClass < cssClassNames.length; iClass++) {
        ...
        if (aHandlerName in structInfo)
          ...
          structInfo[aSecondaryHandlerName].call(
            rval = bindingRootNode.binding, targetBinding, aEvent);
