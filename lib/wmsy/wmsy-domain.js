...
WmsyDomain.prototype = {
   ...
   *
   */
  _focusChew: function(aFocusDef, aStructMap, aProtoFab, aOrigin) {
...
console.log("KEY EVENT!", aEvent.keyCode);
    ...
    switch (aEvent.keyCode) {
      ...
      case 13:
        ...
        if (focusedBinding)
          this._handleEvent(aEvent, focusedBinding.domNode, "on_command");
        break;
      // -- UP
      case 38:
      ...
      for (var iClass = 0; iClass < cssClassNames.length; iClass++) {
        ...
        if (!targetBinding) {
          ...
          if (aHandlerName == "on_click" &&
              ("__clickToFocus" in targetBinding) &&
              targetBinding.__clickToFocus) {
            targetBinding.focus();
            aEvent.preventDefault();
          }
        }
