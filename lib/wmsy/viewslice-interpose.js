...
DecoratingInterposingViewSlice.prototype = {
   ...
   * ]]
   */
  _classifyDudes: function(aItems, aLowKey, aHighKey) {
    ...
    if ((key !== null) && (key != aHighKey) && this._synthLast) {
      generated = this._generator(item, undefined);
      interps.push(new InterposedEntry(effIndex++, key, generated));
      pub.push(generated);
    }
   ...
   *  and only do anything if we are `atTop`/`atBottom` and `_synthFirst`/
   *  `synthLast`.
   *
   ...
   *     The number of deletions that occurred at point
   *   }
   * ]
   */
  _classifyPoints: function(aLowPoint, aHighPoint, aDelCount,
    ...
    // - top
    if (point === 0) {
      if (this._wrapped.atTop && this._synthFirst) {
    ...
    if (point <= aHighPoint && point === items.length) {
      if (this._wrapped.atBottom && this._synthLast) {
    ...
    // -- notify
    if (newPub.length || aDelCount)
      this._listener.didSplice(aLowPoint, aDelCount, newPub,
   ...
   *     `_publicIndexToInternalIndices`.
   *   }
   *   @param[aSilentRangeReduction Boolean]{
   *     This is the result of a `noteRanges` call and thus no `didSplice`
   *     notifications should be generated.  `noteRanges` calls cannot cause
   *     new interposed entries to be created or existing ones to need to be
   *     recomputed, so this is safe.
   *   }
   * ]
   * @return[]
   */
  _exciseRange: function(aLowTrans, aHighTransEx, aSilentRangeReduction) {
  ...
  didSplice: function vs_interpose_didSplice(aIndex, aHowMany, aItems,
    ...
    if (aItems) {
      ...
      this._classifyPoints(aIndex, aIndex + aHowMany, delCount,
                           aRequested, aMoreExpected);
    }
    // -- delete case deferred classification
    // If we deleted anything, we may have invalidated something.
    else if (aHowMany) {
      this._classifyPoints(aIndex, aIndex, 0, aRequested, aMoreExpected);
    }
