...
exports.makeTestPage = function(testHandle, testFunc) {
  ...
  // un-varnished TestRunner means we are in the unit test context and should
  //  use PageWorker
  if (testHandle.__proto__ == ut.TestRunner.prototype) {
    var page = Pages.add(Pages.Page({
      content: pageContent,
      onReady: function() {
        testFunc(page.document);
      }
    }));
  }
  else {
