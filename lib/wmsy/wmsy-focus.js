...
console.log("  parent child class: '" + parentChildClass + "' from",
      ...
      // This loop is handling two cases that look very similar:
      ...
      //    initial position.
      // The symmetry breaking between the two is the state of lastWidget which
      //  is the binding we popped out of.  In the first case lastWidget is
      //  non-null, in the second case it's null.  iterWalk handles the
      //  difference for free, but we really do not want to re-consider the
      //  exact same sub-widget we came out of, so we guard that clause with
      //  a lastWidget check.  (Otherwise we'd never leave the sub-widget!)
      while (curStruct) {
console.log("F^ widget check", curStruct.name);
        if (curStruct.listy) {
          var dis = this;
        ...
        // once we traverse, we no longer have a last-binding for listy iteration
        //  purposes.
        lastBinding = null;
      }
