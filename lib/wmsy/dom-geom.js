...
exports.chewRelPositionDef = function chewPositionDef(aPositioning,
  ...
  return function _position_calc(aPopBinding, aBasisBinding, aExplicitSizing) {
    ...
    if (widg_latch_x == LEFT)
      ...
      tLeft = basisBounds.left + (basisBounds.right - basisBounds.left) / 2;
    else
    ...
    if (aExplicitSizing && aExplicitSizing.height)
      popHeight = cssNormPx(aExplicitSizing.height);
    ...
    if (widg_latch_y == TOP)
      ...
      tTop = basisBounds.top + (basisBounds.bottom - basisBounds.top) / 2;
    else // BOTTOM
    ...
    var popBounds = popNode.getBoundingClientRect();

    var overX = (tLeft + popWidth) - (docWin.scrollX + docWin.innerWidth);
    if (overX > 0)
      tLeft -= overX;
    var overY = (tTop + popHeight) - (docWin.scrollY + docWin.innerHeight);
