...
require.def("wmsy/viewslice-generative",
  [
...
GenerativeViewSlice.prototype = {
  ...
  _batchRequest: function(low, highEx) {
  ...
  grow: function(aDirMagnitude) {
    if (aDirMagnitude < 0) {
      ...
      this._targetBase = 0;
      this._batchRequest(this._exposeBaseIndex, prevLow);
    }
    else {
      ...
      this._batchRequest(prevHigh, this._exposeExclusiveIndex);
    }
  },
  ...
  seek: function(aSeekSpot, aBefore, aAfter) {
    ...
    this.liveList = [];
    this._targetBase = 0;
    this._batchRequest(this._exposeBaseIndex, this._exposeExclusiveIndex);
  },
