...
wy.defineWidget({
  ...
  _structureMapExtra: {
    root: {
      navVertical: null, // tells it to check binding.__listVertical
      prevFocusable: null,
      nextFocusable: null,
      firstFocusable: null, // will be fixed up in protoConstructor
      lastFocusable: null, // will be fixed up
    },

    container: {
      listy: true,
      listVertical: null, // tells it to check binding.__listVertical
      subElementCssClass: "wlib-virt-list-wlib-virt-container-item",
    },
  },
  ...
  protoConstructor: function (aConstraint, aGenesisDomNode) {
    // fix up structure map idempotently
    this.__structMap.root.firstFocusable = this.__structMap.container;
    this.__structMap.root.lastFocusable = this.__structMap.container;
  ...
  receive: {
    ...
    seek: function(aOrderingKey, aFraction, aRelPos, aPadPix) {
      // XXX full coverage for now, we are not clever yet.
