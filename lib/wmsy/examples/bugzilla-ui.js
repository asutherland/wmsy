...
var bvis = require("wmsy/examples/bugzilla-vis");
...
wy.defineWidget({
  ...
  structure: {
    ...
    body: {
      ...
      runBox: {
        runs: wy.libWidget({type: "virt-list",
                            constraint: {type: "bug-run"}}, "runs"),
      }
    },
  },
  // the vis (timeline) and the virt-list (runs) need to talk...
  relay: ["seek", "visibleBindings"],
  style: {
    root : [
      "display: box;",
      "box-orient: vertical;",
  ...
  structure: {
    ...
    absoluteBlock: {
      body: {
        ...
        bodyRight: wy.widgetList({type: "bug-event"}, "events"),
      }
  ...
  events: {
    root: {
      click: function() {
        ...
        var dis = this;
        this.popup_tagMutator(this.obj, function() {
          dis.updateSimilar();
          app.saveState();
        });
