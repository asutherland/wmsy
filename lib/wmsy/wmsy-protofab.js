...
WidgetProtoFab.prototype = {
  ...
  addWidgetList: function(aNodeName, aConstraintBasis, aSeparator,
    ...
    // The sliceAttr stores the underlying content view slice we know about;
    //  we may have even created it.  Which is to say, it is not the decorating
    //  view slice we attached.  The reference is intended to let the binding
    //  code for static view slices get at the view slice to perform list
    //  mutation and for us to use to clean up at destruction-time.  (If we
    //  are interposing, we want to make sure the interposed objects are
    //  destroyed, which means 
