...
FormFieldFakeBinding.prototype = {
  ...
  __focusEnable: function() {
    throw new Error("form fields cannot have individually controllable focus");
  },
  get focused() {
...
WidgetProtoFab.prototype = {
  ...
  addWidget: function(aNodeName, aConstraintBasis, aBindAttr, aDomain,
    ...
    this.proto[setterAttrName] = function $_set(aVal, aForce) {
      ...
      if (widgetFab === null)
        throw new Error("unable to resolve constraint to a widget fab");
    ...
    //  add its own.
    if (!aIsListReuse) {
      this.proto.__destructors.push(function $_destroy() {
        var domNode = this[elemAttrName];
  ...
  addWidgetList: function(aNodeName, aConstraintBasis, aSeparator,
    ...
    // - Undecorated widget list/flow
      ...
      fabBefore = function(aItem, aBeforeNode, aParentNode, aElemOverride) {
        ...
        var widgetFab = partial.evaluate(aConstraintBasis);
        if (!widgetFab) {
          throw new Error("Failure to resolve a widget fab!");
        }
      ...
      splice: function $_sliceListener_splice(aIndex, aHowMany, aItems,
        ...
        if (aHowMany === undefined || aHowMany > 0) {
          ...
          if (aHowMany) {
            ...
            toKill = aSeparator ? ((domNode.children.length || -1) + 1) / 2 :
          ...
          if (aIndex < domNode.children.length)
            nextKidNode = domNode.children[aSeparator ? (aIndex * 2) : aIndex];
          else
            nextKidNode = null;
        }
  ...
  hookupIdSpace: function(domainName, spaceName) {
    this.proto.__initers.unshift(function $_registerBinding() {
      this.domNode.ownerDocument.wmsyIdSpaceDomains[domainName].registerBinding(
        spaceName, this);
    });
    this.proto.__destructors.unshift(function $_unregisterBinding() {
      this.domNode.ownerDocument.wmsyIdSpaceDomains[domainName]
        .unregisterBinding(spaceName, this);
    });
  },
    ...
    this.proto["popup_" + aPopupName] = function popup_$(aObj, aRelBinding,
      ...
      if (aClickAway) {
        var clickAwayListener = function clickAwayListener(aEvent) {
          ...
          binding.done(false, aEvent.target);
          return true;
        };
