...
var domgeom = require("wmsy/dom-geom");
...
var WidgetProto_WidgetList_DOC = {
  ...
  /**
   * Basically the semantics of Array.splice applied to the bound list with an
   *  added flag to indicate whether we are being told about changes or whether
   *  we are also responsible for making the changes.
   *
   * @param["aMakeThisHappen"
   *   @case[true]{
   *        Affect the underlying list object that we are bound to.}
   *   @case[false]{
   *        It is presumed someone else will modify the object or
   *        has already modified it.}]
   * @param["aIndex"
   *   @case[@numgte[0]]{
   *        The starting index for modifications.}
   *   @case[@numlt[0]]{
   *        The number of elements from the back, where -1 is the first,
   *        -2 the second, etc.}
   * @param["aHowManyToDelete"
   *   @case[@jskeyword{undefined}]{Delete until the end of the list.}
   *   @case[@numgte[0]]{
   *         How many items to delete at that index.}]
   * @param["aValues"]{
   *   The list of items to insert at @lx{aIndex}.}
   */
  $_splicey: function(aMakeThisHappen, aIndex, aHowManyToDelete, aValues) {
  },
...
WidgetProtoFab.prototype = {
  ...
  addWidgetList: function(aNodeName, aConstraintBasis, aSeparator,
    ...
    var spliceAttrName = aNodeName + "_splicey";
    var dataAttrName = aNodeName + "_data";

    this.proto[spliceAttrName] = function $_splicey(aIndex, aHowMany, aItems) {
      var domNode = this[elemAttrName], nextKidNode, curKidNode;
      var partial = this[partialAttrName];
      // -- remove bit
      ...
      for (var iItem = 0; iItem < aItems.length; iItem++) {
        ...
        var binding =
          widgetFab.insertBefore(aConstraintBasis, nextKidNode, domNode,
                                 aElemOverride);
      }
    };
...
console.log("E!", scrollOffAttr, scrollTarg);
      containerNode[scrollOffAttr] = scrollTarg;
console.log("E?", containerNode[scrollOffAttr]);
    };

    // this intentionally clobbers the addWidget setter.
    this.proto[aNodeName + "_set"] = function $_set(aVal, aForce) {
      var domNode = this[elemAttrName];
      this[spliceAttrName](false, 0, undefined, aVal);
    };

    this.proto.__destructors.push(function() {
      this[spliceAttrName](false, 0);
    });
  },
   ...
   * @param aAlsoInit Should this also be an initer?  We would want to do this
   *     in the case where all updaters want to be able to assume that this code
   *     has run.  Another option would be to have more of a concept of phases
...
WidgetFactory.prototype = {
   ...
   *     widget.
   * @param [aMakePopup=false] Make the DOM node be absolutely positioned and
   *     mark it with a wmys-popup attribute.  We want to do this before
   *     appending to the tree to avoid causing more layout than is actually
   *     required.
