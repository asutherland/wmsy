...
var WidgetProto_DOC = {
   ...
   *  |_destroy| calls these with the proper 'this' context when invoked.
   */
  _destructors: [],
   ...
   *  just be an alias to the |_destroy| automatically created method.  If the
   *  widget implementation does define this method, it should call the
   *  |_destroy| method before it returns.
   */
  destroy: function() {
...
WidgetProtoFab.prototype = {
  ...
  addEmitter: function(aName) {
    ...
    var handlerName = "_receive_" + aName;
  ...
  addReceiver: function(aName, aFunc) {
    this.proto["_receive_" + aName] = aFunc;
  },
  ...
  makeRendezvousLogic: function(aRelayNames, aEmitNames, aReceiveNames) {
    ...
    this.proto._initers.push(function _init_rendezvous() {
      var i;
      // set up the receiver lists for emitting and relaying
      if (aEmitNames) {
        for (i = 0; i < aEmitNames.length; i++) {
          this["_receivers_" + aEmitNames[i]] = [];
        }
      }
      if (aRelayNames) {
        for (i = 0; i < aRelayNames.length; i++) {
          this["_receivers_" + aRelayNames[i]] = [];
        }
      }
          ...
          if ("binding" in curNode) {
            ...
            if (emitNames && ("__relay" in binding)) {
              for (i = emitNames.length - 1; i >= 0; i--) {
                ...
                if (emitName in binding.__relay) {
                  var receiverListName = "_receivers_" + emitName;
            ...
            if (emitNames && ("__receive" in binding)) {
              for (i = emitNames.length - 1; i >= 0; i--) {
                ...
                if (emitName in binding.__receive) {
                  this["_receivers_" + emitName].push(binding);
                  emitNames.splice(i, 1); // check it off our list
                  if (!emitNames.length) {
                    emitNames = null;
                    if (!receiveNames)
            ...
            if (receiveNames &&
              ...
              for (i = receiveNames.length - 1; i >= 0; i--) {
                ...
                var receiverListName = "_receivers_" + receiveName;
  ...
  /**
   * Focus container hookup.
   */
  makeFocusContainer: function(aVertical, aStructNames) {
    this.proto._focus_vertical = aVertical;
  },
  ...
  makeFocusDomain: function() {
    this.proto._initers.push(function _init_focusDomain() {
      var focusManager = this.domNode.ownerDocument.wmsyFocusManager;
      this._focusDomain = focusManager.newFocusDomain(this);
    });

    this.proto._updaters.push(function _update_focusDomain() {
      var focusManager = this.domNode.ownerDocument.wmsyFocusManager;
      focusManager.ensureDomainFocused(this._focusDomain);
    });

    this.proto._destructors.push(function _destroy_focusDomain() {
      var focusManager = this.domNode.ownerDocument.wmsyFocusManager;
      focusManager.destroyFocusDomain(this._focusDomain);
      this._focusDomain = null;
    });
  },
  ...
  makeFocusable: function() {
    ...
    this.proto._destructors.push(function _destroy_unfocus() {
...
WidgetFactory.prototype = {
  ...
  bindOnto: function(aConstraintBasis, aNode) {
    ...
    if (binding._initers.length){
      var initers = binding._initers;
