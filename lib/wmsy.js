 ...
 *
 */
function TbWmsyExport(aContext) {
  this.useDomain("default");
  this.name = aContext.id;

  this.widgetDefinitions = [];

  // We need a to unregister all of our widget definitions when the time comes.
  let dis = this;
  aContext.addUnloader({
    unload: function() {
      for each (let [, widgetDef] in Iterator(dis.widgetDefinitions)) {
        dis.domain.undefineWidget(widgetDef);
      }
    }
  });
}
TbWmsyExport.prototype = {
...
let WidgetProto_WidgetBase_DOC = {
  /**
...
let WidgetProto_WidgetList_DOC = {
...
let WidgetBaseProto = {
  ...
  _fabFragment: function(aDoc) {
    let frag = this._fragment;
    if (frag)
      return frag.cloneNode(true);

    frag = this._fragment = aDoc.createDocumentFragment();

    let rootMeta = this._structMap.root;

    function fabLevel(aParentMeta, aParentNode) {
      for each (let [, kidMeta] in Iterator(aParentMeta.children)) {
        let node = aDoc.createElement(kidMeta.elementType);
   ...
   */
  _update: function() {
    for each (let [, updater] in Iterator(this._updaters)) {
      updater.call(this);
    }
  },
};
...
WidgetProtoFab.prototype = {
  ...
  addElement: function(aNodeName, aPath) {
    let elemAttrName = aNodeName + "_element";
    ...
    this.proto.__defineGetter__(elemAttrName, function $_element_getter() {
      ...
      for each (let [, iOffset] in Iterator(aPath)) {
  ...
  addWidget: function(aNodeName, aConstraintBasis, aBindAttr) {
    let elemAttrName = aNodeName + "_element";
    let partialAttrName = "_" + aNodeName + "_partial";
    let setterAttrName = aNodeName + "_set";
    ...
    this.proto[setterAttrName] = function $_set(aVal, aForce) {
      let domNode = this[elemAttrName];
      let partial = this[partialAttrName];
      aConstraintBasis.obj = aVal;
      let widgetFab = partial.evaluate(aConstraintBasis);
  ...
  addWidgetList: function(aNodeName, aConstraintBasis, aSeparator,
    ...
    let partialAttrName = "_" + aNodeName + "_partial";
    let itemElemMapAttrName = "_" + aNodeName + "_itemMap";

    let addAllAttrName = aNodeName + "_addAll";
    let clearAttrName = aNodeName + "_clear";

    this.proto[addAllAttrName] = function $_addAll(aItems) {
      let domNode = this[elemAttrName];
      let partial = this[partialAttrName];
      let itemMap = this[itemElemMapAttrName];
      for each (let [, item] in Iterator(aItems)) {
        ...
        if (aSeparator && domNode.lastChild) {
          let sepNode = domNode.ownerDocument.createElement("span");
          sepNode.textContent = aSeparator;
          domNode.appendChild(sepNode);
        }
        let widgetFab = partial.evaluate(aConstraintBasis);
        if (!widgetFab) {
          dump("Failed to resolve widget fab from partial evaluation of:\n" +
    ...
    this.proto._updaters.push(function $_updater() {
      ...
      if (aBindText) {
        if (typeof(aBindText) == "string") {
          ...
          let val = this.obj;
          for each (let [, subAttr] in Iterator(aBindText)) {
            val = val[subAttr];
          }
      ...
      if (aBindAttrs) {
        for each (let [attrName, attrSource] in Iterator(aBindAttrs)) {
          if (typeof(attrSource) == "string") {
            domNode.setAttribute(attrName, this.obj[attrSource]);
          }
          else {
            let val = this.obj;
            for each (let [, subAttr] in Iterator(attrSource)) {
              val = val[subAttr];
            }
  ...
  contribute: function(aObjWithStuff) {
    for each (let [key, value] in Iterator(aObjWithStuff)) {
      this.proto[key] = this.value;
    }
  },
  ...
  makeFactory: function() {
    ...
    if (this.parameterizedByConstraint.length == 0) {
      ...
      let parameterAttributes = this.parameterizedByConstraint;
      let protoCache = this.parameterizedProtos;
      let baseProto = this.proto;
      constructor = function constructor_parameterized(aConstraintBasis) {
        let protoKey = "";
        for each (let [, paramAttr] in Iterator(parameterAttributes)) {
...
WidgetFactory.prototype = {
  ...
  appendChild: function(aConstraintBasis, aParentNode, aElemOverride) {
    ...
    let newNode = doc.createElement(aElemOverride ||
  ...
  insertBefore: function(aConstraintBasis, aBeforeNode, aElemOverride) {
    let doc = aBeforeNode.ownerDocument;
    let parentNode = aBeforeNode.parentNode;
  ...
  replaceBindOnto: function(aConstraintBasis, aNode) {
    ...
    let nextNode = aNode.nextSibling;
    let parentNode = aNode.parentNode;
    parentNode.removeChild(aNode);

    let newNode = doc.createElement(aNode.tagName);
  ...
  bindOnto: function(aConstraintBasis, aNode) {
    let doc = aNode.ownerDocument;
    let binding = new this.constructor(aConstraintBasis);
    let proto = this.proto;
    let frag = proto._fabFragment(doc);

    // We need to make sure the CSS has been injected into the document...
    let wmsyInsertedClasses;
    ...
    if (!(proto.cssClassBaseName in wmsyInsertedClasses)) {
      ...
      if (headTags.length == 0) {
        let headTag = doc.createElement("head");
...
WmsyDomain.prototype = {
  ...
  __structureChewNode: function(aSMap, aKey, aValue, aPath, aProtoFab,
    ...
    // helper tagged things
      ...
      //  perform fusion of nested objects as well.)
        ...
        let constraint = deepSimpleClone(aWidgetDef.constraint);
        for each (let [cKey, cVal] in Iterator(aValue.args)) {
          constraint[cKey] = cVal;
        }
        ...
        let separator = (aValue.opts && ("separator" in aValue.opts)) ?
    ...
    //  future too.
      ...
      let iNodeThisLevel = 0;
      for each (let [subKey, subValue] in Iterator(aValue)) {
        let kidPath = aPath.concat();
  ...
  __styleChewLevel: function(aCssStrings, aStyleLevelDef,
    ...
    for each (let [key, value] in Iterator(aStyleLevelDef)) {
      ...
      if (key == "_") {
        ...
        let nextSelector = aActiveSelector;
        if (key[0] == ":" || key[0] == "[")
          nextSelector += key;
        else
          nextSelector += (nextSelector ? " ." : ".") + aCssClassBaseName + key;

        // flatten E4X that was abused to be a multi-line string...
        if (typeof(value) == "xml")
  ...
  _eventsChew: function(aEventsDef, aStructMap) {
    for each (let [nodeName, handlers] in Iterator(aEventsDef)) {
      if (!(nodeName in aStructMap))
        throw new Error("Unknown structure node: " + nodeName +
                        " to bind event to!");

      let structMeta = aStructMap[nodeName];
      for each (let [eventName, eventHandler] in Iterator(handlers)) {
...
dump("WIDGET DEF: " + aWidgetDef.name + "\n");

    let protoFab = new WidgetProtoFab(this, aOrigin, aWidgetDef);

    let structureMap = this._structureChew(aWidgetDef.structure, protoFab,
                                           aWidgetDef);
    ...
    let cssClassToStructInfo = protoFab.proto._cssClassToStructInfo = {};
    let globalCssClassToStructInfo = this.cssClassToStructInfo;
    for each (let [, meta] in Iterator(structureMap)) {
      cssClassToStructInfo[meta.cssClassName] = meta;
      globalCssClassToStructInfo[meta.cssClassName] = meta;
    }

    let cssBlocks = this._styleChew(aWidgetDef.style,
                                    protoFab.proto.cssClassBaseName);
    protoFab.proto._flattenedCssString = cssBlocks.join("\n");


    let widgetFactory = protoFab.makeFactory();
    this.dtree.addPossibility(aWidgetDef.constraint, widgetFactory);

    this.dtree.build();
dump("  END WIDGET DEF: " + aWidgetDef.name + "\n");
  },

  undefineWidget: function(aWidgetDef) {

    this.dtree.removePossibility(aWidgetDef);
dump("REMOVED WIDGET, REBUILDING\n");
    this.dtree.build();
dump("  REBUILT\n");
  },
  ...
  emit: function(aConstraintWithObj, aDomElem) {
    let widgetFab = this.dtree.evaluate(aConstraintWithObj);
  ...
  bindEventsOnRoot: function(aRootElem) {
    ...
    aRootElem.addEventListener("click",
                               function(aEvent) dis._handleEvent(aEvent),
                               false);
  },
  ...
  _handleEvent: function(aEvent) {
    let handler_name = "on_" + aEvent.type;
    // find and latch the most specific binding...
    let targetBinding = null;
    for (let curNode = aEvent.target; curNode; curNode = curNode.parentNode) {
      let cssClassName = curNode.getAttribute("class");
dump("curNode: " + curNode + " : class " + cssClassName + "\n");
      // keep looking if it's not ours
      if (!(cssClassName in this.cssClassToStructInfo))
        continue;
dump("  found struct info!\n");
      let structInfo = this.cssClassToStructInfo[cssClassName];

      // if we have not yet latched the target binding, figure it out now
      if (!targetBinding) {
        targetBinding = curNode;
        for (let i=structInfo.path.length; i >= 0; i--)
...
let TbWmsy = {
  ...
  getDomain: function(aName) {
    ...
    let domain = new WmsyDomain(aName);
...
function test_css() {
  ...
  let domain = new WmsyDomain("test");

  for each (let [, [testObj, expected]] in Iterator(cssMapifications)) {
    let blobs = domain._styleChew(testObj, cssPrefix);
    teq(blobs.length, 1);
    let blob = blobs[0].replace("\n", "", "g").replace(/ +/g, " ");
    teq(expected, blob);
  }
}
//test_css();
